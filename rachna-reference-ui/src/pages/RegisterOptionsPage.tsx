...
export const RegisterOptionsPage: React.FC = () => {
  ...
  return (
    <KioskLayout
      showBack={true}
      backTo="/auth"
      audioText={`${t.registerOptionsTitle}. ${t.registerOptionsSubtitle}. ${t.regAbhaTitle} or ${t.regPhoneTitle}.`}
    >
      <ScreenTransition className="max-w-5xl mx-auto py-2">
        ...
        <div className="text-center mb-8 md:mb-12">
          <h1 className="text-4xl md:text-5xl font-extrabold text-navy-900 tracking-tight mb-3">
            {t.registerOptionsTitle}
          </h1>
          <p className="text-xl md:text-2xl font-semibold text-slate-500 tracking-normal">
            {t.registerOptionsSubtitle}
          </p>
        </div>

        {/* Two Giant Cards: Existing ABHA ID vs Phone Number (Matching Mockup Screen 4) */}
        <div className="w-full flex flex-col md:flex-row items-center justify-center gap-6 md:gap-8 relative px-4">
          {/* LEFT: Existing ABHA ID */}
          <ActionCard
            icon={CreditCard}
            title={t.regAbhaTitle}
            description={t.regAbhaDesc}
            variant="green"
          ...
          <ActionCard
            icon={Smartphone}
            title={t.regPhoneTitle}
            description={t.regPhoneDesc}
            variant="green"
            onClick={handleChoosePhone}
            badgeText="New Profile"
          />
        </div>
      </ScreenTransition>
    </KioskLayout>
